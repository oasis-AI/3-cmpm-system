...
flowchart TB
	Mall["中国移动积分商城系统"]
	Mall --> UserSide["用户端模块"]
	Mall --> MerchantSide["商户端模块"]
	Mall --> AdminSide["管理员端模块"]

	UserSide --> U1["首页推荐与公告浏览"]
	UserSide --> U2["分类检索与商品详情"]
	UserSide --> U3["购物车与积分兑换"]
	UserSide --> U4["快捷充值与每日签到"]
	UserSide --> U5["订单中心与地址管理"]
	UserSide --> U6["积分流水与活动参与"]

	MerchantSide --> M1["商户入驻与资质维护"]
	MerchantSide --> M2["商品管理与 SKU 维护"]
	MerchantSide --> M3["库存预警与库存调整"]
	MerchantSide --> M4["订单处理与物流发货"]
	MerchantSide --> M5["经营概览与活动参与"]
...
```mermaid
flowchart TD
	A["用户选择商品、SKU、数量和地址"] --> B["校验商品状态与规格合法性"]
	B --> C{"可用库存是否充足"}
	C -- 否 --> C1["返回库存不足提示"]
	C -- 是 --> D{"积分余额是否足够"}
	D -- 否 --> D1["返回积分不足提示"]
...
```mermaid
flowchart TD
	A["用户输入手机号并选择充值类型、套餐"] --> B["校验手机号格式与套餐合法性"]
	B --> C{"积分余额是否足够"}
	C -- 否 --> C1["返回积分不足提示"]
	C -- 是 --> D["计算所需积分"]
	D --> E["创建 phone_recharge_orders 记录"]
	E --> F["扣减用户积分并写入积分流水"]
	F --> G["生成充值订单结果"]
	G --> H["用户在个人中心查看充值记录"]
```
...
```mermaid
flowchart TD
	A["订单创建完成，状态为 paid"] --> B{"后续业务处理"}
	B -- 商户发货 --> C["填写物流公司与物流单号"]
	C --> D["订单状态更新为 shipped"]
	D --> E{"用户操作"}
	E -- 确认收货 --> F["订单状态更新为 completed"]
	F --> G["释放对应锁定库存"]
	E -- 申请退款 --> H["生成 refund_requests 记录"]
	B -- 申请退款 --> H
	H --> I["订单状态更新为 refunding"]
	I --> J["管理员审核退款申请"]
	J --> K{"审核是否通过"}
	K -- 通过 --> L["返还用户积分并新增 refund 流水"]
	L --> M["订单状态更新为 refunded"]
	K -- 拒绝 --> N["退款申请置为 rejected"]
	N --> O["订单状态恢复为 completed 或 shipped"]
```
...
| total_cash | DECIMAL | 10,2 | - | 现金补差价（默认0） |
...
需要说明的是，图 5-2 所示的用户首页中包含商品分类导航、商品列表、活动展示区、公告滚动条和快捷入口等全部功能板块，这些均为中国移动积分商城系统的组成部分，而非某个更大平台内部的独立子模块。本次毕业设计的工作范围即为该系统的完整设计与实现，首页所呈现的全部功能均在本系统中实现。
